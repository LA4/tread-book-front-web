...
export default function BooksDetails({ params }: { params: { slug: string } }) {
  ...
  const fetchDeleteBook = async (id: number) => {
    const response = await fetch(
      `${API_THREADBOOK}/books/delete/${id}/${user.user_id}`,
      { method: "DELETE" }
    );
    const data = await response.json();
    router.replace("/home");
  ...
  return (
    <div className="flex flex-col justify-center items-center h-full w-screen text-white my-[80px]">
      {bookData && (
        <form
          className="flex w-full justify-center items-center flex-col gap-2 p-2"
        ...
        >
          ...
          <div className="flex flex-col gap-8 justify-center items-center m-8">
            <div className="flex items-center justify-center">
              <input
                ...
                {...register("pageRead")}
              />
              <span className="flex w-1/5 text-olive-dark">
                / {bookData.pages}
              </span>
            </div>
            ...
            <select
              {...register("status")}
              className="text-charcol flex bg-transparent  p-2 rounded-lg border-2 border-olive-dark  text-center"
            >
              <option
                className="text-charcol bg-gray"
                value="CURRENTLY_READING"
              >
                Currently Reading
              </option>
              <option className="text-charcol bg-gray " value="TO_BE_READ">
                T B R
              </option>
              <option className="text-charcol bg-gray " value="READ">
                Read
              </option>
            </select>
          </div>
          ...
          <input
            id="title"
            className="text-center bg-transparent flex p-1 w-full rounded-t-lg  mb-4 text-charcol border-b-2 "
            {...register("title")}
          />
          ...
          >
            <span className="flex text-[.8rem] w-1/2 text-charcol m-2">
              Author :
            </span>
            <input
              className="w-1/2 text-center bg-transparent flex p-1 rounded-t-lg w-full text-[.8rem] text-olive-dark border-b-2 h-6"
              {...register("author")}
            />
          </label>
          ...
          {openResume ? (
            <div className="flex flex-col w-full">
              <textarea
                className="flex p-2 h-[300px] bg-white text-charcol"
                {...register("resume")}
              />
              <span
                onClick={() => {
                ...
                className="bg-olive-dark justify-center flex p-1"
              >
                {" "}
            ...
            <div className="flex flex-col w-full">
              <div className="flex p-2 h-[100px] w-full bg-beigr-light text-olive-dark overflow-hidden ">
                {bookData.resume}
              </div>
              <span
                onClick={() => {
          ...
          <label
            htmlFor="opinion"
            className="flex w-[80%] flex-col font-bold"
          ></label>
          <span className="text-charcol"> What about ?</span>
          <textarea
            id="opinion"
            className="flex p-2 h-[200px] w-full rounded-t-lg text-charcol m-1 bg-white border-2 border-olive-light text-charcol"
            {...register("opinion")}
          />
          <div className="flex justify-between w-full px-8">
            {!openModal && (
              <span
                className="flex h-[30px] cursor-pointer items-center text-[.8rem] text-gray font-bold underline underline-offset-4"
                onClick={() => {
